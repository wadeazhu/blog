...
![img](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1603966294889-153271b5-4b9f-4470-b05f-c7a9f030d043.png)
...
实现懒加载的第二种方式，上面这种方式，因为时间的频繁调用和重复获取`dom`元素的距离，导致出现性能问题。可以使用浏览器提供的观察者`api`来代替`scroll`事件,`Intersection Observer API`
...
实现懒加载的第三中方式，浏览器对`img`新增了一个`loading`属性，当等于`'lazy'`时，可以做到懒加载的效果，但是兼容性现在不太友好

```html
<img src="./1.png" loading='lazy' />
```

![image-20240704151733614](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/image-20240704151733614.png)
