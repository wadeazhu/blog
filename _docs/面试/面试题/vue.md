...
![0_tB3MJCzh_cB6i3mS-1.png](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1620128979608-f7465ffc-9411-43e3-a6bc-96ab44dd77df.png)
...
![img](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1618656573096-ebdc520c-5d60-4d12-ad04-5df4ebbb5fe7.png)
...
![image.png](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1603814137582-5a9aa62f-0045-4272-bef0-447dedb25596.png)
...
![image.png](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1603814104939-8c8ac923-735d-4476-937a-cb1f795ffe84.png)
...
- exclude 字符串或正则表达式，任何名称匹配的组件都不会被缓存，优先级高于include；
- max 数字，最多可以缓存多少组件实例。
...
![img](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1604019269329-d88e91cf-b33d-4b2d-b014-e5739e9b7dbc.png)
...
![img](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1609521413572-54d0bd0f-8ed6-4438-997a-c890e4cd9c5e.jpeg)
...
![img](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1609518480272-8cb1af01-a4a8-4d54-91bb-5546aafac510.jpeg)
...
vue2中， v-for优先于v-if被解析，如果同时出现，每次渲染都会**先执行循环再判断条件**，无论如何循环都不可避免，浪费了性能。
...
### 48.watch和computer和create的顺序

- 如果watch immediate:false
  - beforeCreate create beforeMount computer mount watch
- 否则
  - 如果watch监听的是computer的属性，那么computer在前
    - computer watch create mount
  - 否者
    - watch computer create mount
...
![img](https://azhu-images.oss-cn-hangzhou.aliyuncs.com/img-for-marktext/1609522434579-ff590978-32e1-4cf5-bbbd-d8caf932f8d6.png)
...
1. 更轻量化， 只有1kb的大小
2. 更简洁的写法。可以直接修改state上的属性。
3. 去掉了mutation，直接actions写同步和异步的函数。
4. **强大的自动补全**：Pinia 为 TypeScript 提供了强大的支持，包括针对 JavaScript 的强大自动补全功能
5. pinia支持服务端渲染（ssr）
...
3. 使用`ref`声明的数据，在JS中使用的时候，需要添加`.value`的后缀；
4. `reactive`函数只能接收引用数据类型；
5. `ref`底层还是使用`reactive`来做，`ref`是在`reactive`上进行了封装，增强了其能力，使其支持了对原始数据类型的处理；
6. Vue3中，`reactive`能做的`ref`能做，`reactive`不能做的`ref`也能做；
...
在写路由时候增加的权限的编码，然后再登录的时候获取用户的权限，再结合两者比对，就可以获取该用户的菜单。再通过路由守卫进行第二次判断。

1. 写菜单路由时，在meta属性中增加权限的关键字
2. 在登录页面的时候获取该用户的权限信息
3. 结合router和权限信息获取该用户的菜单
4. 生成动态的权限菜单
5. 在路由守卫中，写权限逻辑
...
在写路由的时候，只需要写基本的路由。用户的页面组件路径保存后端，根据后端返回的路由地址，动态拼接route对象。
...
获取权限后，通过v-if手动判断写具体逻辑

### 2.如何实现类似antd中alert组件写法。
